...
.import "./textSearch.js" as TextSearch_

const gameData = GameData.GameData.json;
const TextSearch = TextSearch_.TextSearch;
...
class Text
{
  constructor()
  {
    ...
    const text = JSON.parse(gameData("text"));
    this.rawTrans = text;
    for (let i = 0; i < text.length; i++) {
      // Get translate pair entry
      const transPair = text[i];
  ...
  get search()
  {
    return new TextSearch(JSON.parse(gameData("text")));
  }
  ...
  // Possibly very slow
  convertToCode(str, maxLength = 10, autoEnd = true) {
    let code = [];
    let lastCode = 0;
  ...
  convertFromCode(codes, maxLength = 10) {
    let eng = "";
  ...
  // in-game
  convertEngToHTML(msg, maxChars, rival = "BLUE", player = "RED") {
