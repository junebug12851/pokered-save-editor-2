...
| `notes/reference/filter-flags.md` | **Filter flags — all 228 missable lifecycles.** The chain: **event flag (durable fact) → map script (the reconciler, often per-entry) → filter flag (what the renderer consults)** — so many bits are RE-DERIVED on map load (both Snorlax, the Cerulean rival, Silph's receptionist, the boulders). Five classes: **121 static** (items/static encounters, engine-hidden once), **44 sweep** (all Silph-liberation), **36 one-shot** (swap pairs, gift removals, departures — each with its exact trigger routine), **14 re-armed** (VR + Seafoam boulders, outside-rewritten per approach), **13 cutscene/per-entry visitors** (Oak's shape; Pewter's draggers reset every visit). **Oak-in-Pallet exemplar**: default hidden, shown by the Hey-Wait intercept, re-hidden by the ENGINE (`auto_movement.asm`) — every resting stage holds him hidden. Tool: `scripts/analyze_filter_flags.py`. **Read before missable UI/matching/randomizer work** |
