...
## Map-state blueprints — a NEW format, argued for (2026-07-17)

**Decision**: per-map progression blueprints ship as our own JSON
(`projects/db/assets/data/map-states/<Map>.json`), generated by `scripts/extract_map_states.py`
from `pret/pokered`'s `scripts/<Map>.asm` with a curated overlay.

**Why this is a permitted deviation from the "use the game's own format" rule**: pret has **no
format for this data — it does not exist as data at all.** A map's progression states live only
implicitly in executable assembly (`CallFunctionInTable` dispatch + `SetEvent`/`HideObject` calls
scattered through routine bodies). The blueprint is a *computed research product* (which values are
resting stages, what each stage means, what the condensed save-file delta is), the same class of
derived artifact as `events.json`'s dossiers. The pret source remains the single upstream: the
extractor parses their `.asm` verbatim (their macro names are the grammar — `dw_const`,
`SetEvent`, `predef HideObject`), is `--check`-idempotent, and every referenced id must resolve
against the shipped vocabularies (`events.json`, `missables.json`, `scripts.json`, `maps.json`) or
generation fails.

**Rejected**: shipping the raw `.asm` and interpreting at runtime (the app would need an LR-35902
