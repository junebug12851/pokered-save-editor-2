...
|-------|-------|----------------|
| Randomizer: **map/area randomization disabled** until maps are set up | `savefileexpanded.cpp`, `worldgeneral.cpp` | **Working as of 2026-06-07 (Twilight-authorised).** `randomizeExpansion()` now runs end-to-end and is test-covered (`tst_randomizer` 10-iteration invariants + `tst_verbs` byte-fidelity, both green). The map-dependent randomizers are commented out because maps aren't wired yet: `area->randomize()` (in `SaveFileExpanded::randomize`) and the two map picks in `WorldGeneral::randomize` (`isCity()`/`isType("Outdoor")`). Bugs found+fixed along the way: `MapSearch::isType()` null-deref (deref of null `toTileset`); `HoFPokemon::load()` null-deref (dereferenced `saveFile` before its null check — also crashed `HoFRecord::pokemonNew`/`new HoFPokemon`). **To re-enable map randomize (future map feature):** fix the tileset type strings (`isType("Cave")`/`isType("Outdoor")` currently match **0 maps**), guard/repair `SpriteData::load()` on unresolved `getToSprite()` links, and guard the `pickRandom()->getInd()` null cases in `area/*` + `worldgeneral`. Minor: `HoFPokemon::randomize()` looks up `getIndAt("dex"+N)` (not a real key) → HoF mons randomize to species 0 (cosmetic; not a crash). |
