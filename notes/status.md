...
A comprehensive automated test suite now exists under `projects/tests/` (QtTest + CTest, **49
executables, all green** — last full run re-verified 2026-06-08 on the real Qt 6.11
llvm-mingw kit: clean rebuild, `ctest` 49/49 including the new `tst_pokemonbox`). A **coverage-gap-fill pass
toward the 100% target is underway** — savefile real-source line coverage is now **82.4%**
(`pokemonbox.cpp` 72%→93.9% this pass); see `plans/testing.md` for the live savefile gap list and
