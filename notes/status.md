...
|-------|-------|----------------|
| Randomizer (`randomizeExpansion()`) crashes on a progressed save | `area/*` randomize paths | **OPEN — for phase 7 (randomizer is WIP).** `tst_verbs` found `randomizeExpansion()` crashes: (1) `MapSearch::isType()` null-deref (deref of null `toTileset`) — **fixed** 2026-06-07; (2) still crashes further in `SpriteData::load()` via `AreaSprites::randomize()` (null/uninit `MapDBEntrySprite`). The randomize byte-fidelity test is `QSKIP`'d until phase 7 untangles the crash chain + invariants. `resetData()` and `eraseExpansion()` fidelity both pass (neither touches a save byte). Also (3) `MapSearch::isType("Cave")` matches **0 maps** (the cave tileset type string isn't literally "Cave"), so the randomizer's `isType("Cave")->pickRandom()` would return null — same phase-7 fix. |
