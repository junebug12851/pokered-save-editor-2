...
A comprehensive automated test suite now exists under `projects/tests/` (QtTest + CTest, **50
executables, all green** — last full run re-verified 2026-06-08 on the real Qt 6.11
llvm-mingw kit: clean rebuild, `ctest` 50/50 including `tst_pokemonbox` + `tst_sprite_data`). A
**coverage-gap-fill pass toward the 100% target is underway** — savefile real-source line coverage is now
**86.5%** (`pokemonbox.cpp` 72%→94.4%, `spritedata.cpp` 46%→100% this pass); see `plans/testing.md` for
