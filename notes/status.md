...
`tst_area_logic`). A **coverage-gap-fill pass toward the 100% target is underway** — savefile real-source
line coverage is now **90.2%** (this pass: `pokemonbox.cpp` 72%→94.6%, `spritedata.cpp` 46%→100%,
`areatileset.cpp` 62%→100%, `areamap.cpp` 62%→93%, `areapokemon.cpp` 61%→90%, `areawarps.cpp` 70%→99%);
see `plans/testing.md` for the live savefile gap list and per-file progress. The app logic was extracted into a static `appcore` library
