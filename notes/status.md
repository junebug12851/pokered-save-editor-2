...
> **the project now compiles, links, runs, AND the maintainer confirmed runtime parity with the
...
> exists only on disk. Key lesson (the maintainer): **diagnose against git history,
...
> **Next: the maintainer must COMMIT/BACK UP now (builds+runs; the prior backup was the corrupted one), then
...
**Save-fixture corpus reorganised (2026-06-13):** all test saves now live under `assets/saves/`
in three categories — **`natural-clean/`** (game-generated saves, real hardware or emulator:
`BaseSAV.sav`, `BaseSAV.new.sav`), **`synthetic-clean/`** (engine-built well-formed saves, was
`assets/synthetic/`), and **`synthetic-dirty/`** (malformed/garbage saves, was `assets/dirty/`).
...
real SemVer pipeline with a **single source of truth** — repo-root **`VERSION`** (`0.7.0-alpha`).
...
`tst_qml_screens`). **To bump:** edit one line in `VERSION` + reconfigure. Releases get a `vX.Y.Z`
git tag (on request).
...
relinked w/ the translations qrc; app launched). **Language switching is deferred** (the maintainer's
...
`decisions/architecture.md` → "i18n via Qt Linguist". **Awaiting the maintainer's in-app review**
...
`Q_INVOKABLE` returns via `qmlCppOwned`). The maintainer confirmed: data flows, names render, no crash when
clicking around.

We are in a **UI-polish phase**. Two big screens are now polished + maintainer-iterated:

1. **Pokémon details editor** (General / DV-EV / Moves tabs + Glance pane) — sessions 13k–13t, the maintainer
...
2. **Name editors — full keyboard + quick-edit popup** (`name-full/*`, `general/NameDisplay.qml`) —
   sessions 13v–13z8, heavily iterated with the maintainer live. Now: wide centered name box; a **"Simulated"**
...
labels rode high). All maintainer-driven, live-iterated.

**Next:** continued the maintainer review of the name editors; an end-to-end save/reopen verification pass. See
`plans/next-steps.md`.
...
**Drag & drop — iterated in-app with the maintainer (2026-06-09, built + storage tests green, app relaunched):**
...
(overlay, no reflow; the maintainer rejected the full-cell box and any live entry-shuffle); the "+" slot's caret
marks "after the last mon, before New". (3) **footer bulk-action bar removed** (`PokemonPane.qml`) — moves
are drag now. (4) **checkbox selection has scoped persistence** (the maintainer's rule): survives **only** the
...
**Pokémon box-header UX pass (2026-06-10, maintainer-directed; built clean in repo `build/`, KIT-DIR
REBUILD pending for in-app review):** Reworked the two affordances on the Pokémon screen's box
...
(`projects/build/Desktop_Qt_6_11_0_llvm_mingw_64_bit-Debug`) for the maintainer to test in-app. Convention:
`reference/ui-patterns.md` → "Pokémon storage screen layout".

**Credits screen — now living + AI contributors added (2026-06-10, maintainer-directed):** Added a new
...
The maintainer can move "AI Assistance" — it's just the `process()` append order.)

**Items / Bag screen drag & drop (2026-06-10, maintainer-directed; BUILT + full `ctest` green 57/57,
kit-dir rebuilt + app launched for in-app review):** Brought the same drag interaction the Pokémon
...
controls must keep their clicks (the maintainer chose the grip over press-hold); reorder within a list +
**cross-pane transfer** (bag↔PC) via drag; the **footer bulk-action bar was removed** (match Pokémon);
a **per-row delete chip** sits to the right of the count field, `visible: hover || checked` (the maintainer's
...
**Awaiting the maintainer's in-app review** (watch: grip/checkbox column alignment under the header check-all,
...
maintainer-directed; BUILT + full `ctest` green 57/57, kit rebuilt + app relaunched):** Follow-ups to the
...
room for that row, the transfer is **refused** (item stays put). Never clamps/loses items (the maintainer's
...
**Items delete chip — no rest background + scrollbar clearance (2026-06-10, maintainer-directed; QML-only,
...
kit rebuilt anyway for review). **Follow-up (same day):** the chip used `visible:` which collapsed
...
**Items "View All" overview drawer (2026-06-10, maintainer-directed; BUILT + full `ctest` green 57/57, kit
...
**maintainer-iterated to done (same day):** started as a Material `Drawer` but it kept leaving a white
...
hover delete chip showing through); rebuild `onShownChanged`. The maintainer signed off.

**Trainer-card badges normalized to uniform square canvases (2026-06-12, maintainer-directed; BUILT + full
...
**The maintainer approved 2026-06-12.** Credits updated: ChatGPT credited (AI Assistance + Icons) for the
gym-badge images and the gym leader portraits. (If a different badge margin is wanted later, re-run the
normalize at a different fill fraction; 98% chosen to match the leader portraits.)

**Trainer-card leader shadows recropped to square busts (2026-06-12, maintainer-directed; BUILT + full
`ctest` green 57/57 + app relaunched):** The maintainer recropped all 8 gym-leader shadows to **256×256 square
...
**The maintainer approved 2026-06-12.**

**Trainer-card badge sizing/aspect fix + optimized badge PNGs (2026-06-12, maintainer-directed; BUILT +
full `ctest` green 57/57 + app relaunched):** Two follow-ups to the badge swap below. (1) Re-imported the
**8 badge icons** after the maintainer optimized them (~1 MB → ~360–430 KB each, ~3 MB total now vs ~8 MB);
...
Convention note added below. **The maintainer approved 2026-06-12.**

**Trainer-card badge artwork swap (2026-06-12, maintainer-directed; BUILT + full `ctest` green 57/57 +
...
baked into the binary) — easy win later to downscale them to display size if the maintainer wants. Convention
unchanged. **Awaiting the maintainer's in-app review.**

**Trainer-card artwork swap (2026-06-10, maintainer-directed; BUILT + app relaunched for in-app review):**
...
launched. `red-larger.png` left in place (no longer referenced; safe to remove later if the maintainer
wants). Note: the new art is a tall portrait, so within the `parent.width/3` box it fits to height
and reads fairly narrow — easy to widen later if needed.

**Pokémon "View All" overview drawer (2026-06-12, maintainer-directed; BUILT + full `ctest` green
...
by species name, not nickname), **columns = Party first then only NON-EMPTY boxes** (the maintainer's call —
...
drawer (Pokémon screen)". **Awaiting the maintainer's in-app review** (watch: column widths/labels when
several boxes are non-empty, the horizontal scroll on a many-box save, and the tooltip wording/cramping).

**Pokémon "View All" polish pass (2026-06-12, maintainer-directed; BUILT + full `ctest` green 57/57 + test
...
the maintainer's in-app review.**

**Pokémon "View All" — sort icons + condensed columns (2026-06-12, maintainer-directed; BUILT + full
...
tooltip looked bad/cluttered). The maintainer added three PNGs (`sort-alphabetical/internal/pokedex`); copied
...
-columns bullets). **Awaiting the maintainer's in-app review** (watch: the three sort icons' relative visual size
under `PreserveAspectFit`, and box-number-only headers reading clearly enough).

**Pokémon storage: grid name-fix + Tools menu / Boxes-Formatted warning (2026-06-12, maintainer-directed;
...
the maintainer's in-app review** (watch: Tools menu position above the footer, and the warning copy/wording).

**Rival screen cleanup — matched to the Trainer Card layout (2026-06-13, maintainer-directed; BUILT +
...
the name — elements anchored below one another (no fixed/negative offsets). The maintainer chose to **keep the
...
**The maintainer approved the card-box approach up front; awaiting in-app review of the result.**
...
illustration is the same lineage but remains historically uncredited.) Standing ask from the maintainer: **keep
credits/sources current proactively** — flag and add credits for new assets/contributors by default.

**Credits / About screen redesign + data/back-end cleanup (2026-06-13, maintainer-directed; BUILT +
full `ctest` green 57/57, kit rebuilt + app launched):** Reworked the About/Credits modal end-to-end
while keeping what the maintainer liked (wallpaper, categories, font-size variation). (1) **Data:**
...
JSON edit (no C++). Entries preserved verbatim (incl. The maintainer's wording/typos). (2) **Back end:**
...
`.version` → "Pokered Save Editor • v1.0.0", set in `boot.cpp`; "© 2017–2026 the maintainer • Apache License
...
About screen". **Awaiting the maintainer's in-app review** (watch: card width/shadow on wide windows; the
...
lesson (the maintainer, valid):** these are exactly what a **QML load/GUI smoke test** would catch automatically,
...
**Second comprehensive pass (2026-06-13, maintainer-directed "I really need good comprehensive testing"):**
...
byte-identical (over progressed + new + a synthetic maxed save). (2) **`assets/saves/synthetic-dirty/` + `tst_dirty_fixtures`** — malformed saves
...
index → `boxes[121]`; see Open Issues). (3) The Pokémon editor verification above. (4) **`assets/saves/synthetic-clean/`
+ `gen_synthetic_fixtures` + `tst_synthetic_fixtures`** — engine-generated deterministic edge-case saves
(maxed/zeroed/allbadges/midgame, built from a FRESH new file so no real data; real saves stay the maintainer's),
...
| Name editors — the maintainer's continued review | `name-full/*`, `general/NameDisplay.qml` | Open: ongoing live tweaks. `NameEdit`/`NameDisplay` shared by player/rival/nickname + keyboard footer — verify all on each rebuild. |
| Full keyboard: right-side `DetailView` (text info on hover) | `name-full/DetailView.qml` | Still present + wired alongside the pill tooltip. Confirm it still reads well / whether the maintainer wants to keep it. |
| ~~Trainer-card box size / field spacing / clock width / label centering~~ | `TrainerCard.qml`, `CardFront.qml`, `PlaytimeEdit.qml`, `DefTextEdit.qml` | **Done + maintainer-confirmed (s13z10–z11).** Centered box restored (`500×250`); `CardFront.fieldH` (28) compacts every field; playtime narrowed (`digitPad`) + row pinned to `fieldH` so digits/`:` vcenter; `DefTextEdit` label now vcenters. Tuning knobs in `ui-patterns.md`. (Horizontal text padding inside the non-clock fields still at `DefTextEdit` default — trim per-instance only if the maintainer wants it tighter.) |
...
(dark text, no background — 2026-06-08, was hover-only; pending the maintainer review). The player **ID**
commits on Enter/focus-out (not per keystroke) — the maintainer can revert if they want it live.
...
| Trainer Card / Bag / Pokémon storage data | ✅ the maintainer confirmed |
...
| Trainer-card layout — centered box, compact fields, clock width, label centering | ✅ maintainer-confirmed (s13z10–z11): `SwipeView` 500×250; `CardFront.fieldH`; `PlaytimeEdit` `digitPad` + row pinned to `fieldH`; `DefTextEdit` label vcentered |
| Randomize name (full editor + trainer screen) | ✅ (s13e) |
| Keyboard filter / description / random / button styling | ✅ (s10–11) |
| Pokémon **editor** responsiveness (species/level/status/HP, moves, DV/EV, shiny, menus) | ✅ Fixed + maintainer-confirmed (s13l) |
| Pokémon **editor** layout + styling (GridLayout/RowLayout, heights, borderless combos w/ hover, square move pills, ⋮ buttons, slider hover tooltips) | ✅ Polished + maintainer-confirmed (s13m–s13t) |
...
| **Full keyboard** — name box, Simulated group (Outdoor/Tileset/Grid-Tileset), radio filters, pill grid + tile tooltip, Name/Example toggle | ✅ Reworked + maintainer-iterated (s13v–s13z8) |
