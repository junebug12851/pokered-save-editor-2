...
| Gotcha | One-line |
|--------|----------|
| **An INCREMENTAL build can lie about the opaque-pointer flag** | The `IsPointerToTypeDerivedFromQObject` flag is baked into the TU that *reads* the property. On 2026-07-12, restoring the `#include` for `AreaAudio` and rebuilding **still failed** the chain test — a stale object was still carrying the old flag; only wiping `savefile.dir` and rebuilding made it pass. **After changing an include / `Q_DECLARE_OPAQUE_POINTER` that affects a QMetaType flag, force a full rebuild before believing either a pass OR a fail.** |
