...
the primary record, and `version-notes.md` shows where several facts were figured out (e.g. the
...
- **Three nested grid units (now resolved).** Gen 1 maps use three sizes, smallest to largest:
  ...
  tiles" note was wrong — Twilight's "not completely sure" instinct was right. **Standardize
