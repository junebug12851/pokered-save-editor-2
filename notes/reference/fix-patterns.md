...
| `AreaPokemon::setTo()` could overflow the fixed wild arrays | The loops wrote `grassMons[i]`/`waterMons[i]` for `i < monData.size()` with no `i < wildMonsCount` guard. Gen-1 tables are always exactly 10 so it can't overflow today, but the guard was added defensively (`&& i < wildMonsCount`). (2026-06-08.) |
