...
```
**"Label + field" row that stays aligned at any Material height** (June's "option #2"): a `RowLayout`
...
`topMargin` rows. (June's standing preference: proper layouts, no margin hacks — see
...
4. **If a compact field is wanted everywhere**, the shared base is `fragments/general/DefTextEdit.qml`
   ...
   screens at once — but it changes field appearance globally, so it's June's call (she owns UI).
...
is **nothing to flick** — it just clips at the screen edge and rubber-bands (exactly June's "bounces
