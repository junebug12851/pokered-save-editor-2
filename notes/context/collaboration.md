# Working On This Project — Preferences, Standing Rules & Cross-Cutting Feedback
...
start and keep them current; do not stash project knowledge in any external/personal memory** (project
leadership, 2026-07-10). This file consolidates that knowledge so nothing is trapped in a side channel.
Where a rule has a deeper home, it's cross-referenced.

---

## Describe what the EDIT DID — not what the world now looks like (2026-07-17)

The request was specific: disable the camera view box by default — the outline around the player that is
exactly the Game Boy screen view. Exactly one bit changed (`ViewScreenBox`). But the comment, the test,
the changelog and the chat reply all said:

> ~~"So **both** camera-ish boxes that ride the player (Screen box and Draw area) now start off"~~

**True about the state. False about the change.** The Draw area had been off since `3a22f84` — a prior
leadership decision — and was never touched. It read as a second layer being switched off unasked, which
raised the obvious question: what else got disabled? It also flatly contradicted what had been reported
earlier **in the same session**, which is worse than being wrong once: it makes the true statement look
unreliable too.
...
it reads as unrequested scope. That is a live concern here, and a well-earned one: **"don't build what
hasn't been briefed"** is a standing rule, so prose that invents extra scope undermines the thing that
most needs to be trustworthy. If the state is worth mentioning, mark it plainly:
...
- **Address the ROLE, not a person — "project leadership" (updated 2026-07-15).** Frame all decisions
  and sign-offs as **"project leadership"** in notes, commits, and conversation — act as you would with
  a team of several owners, whether there is one active contributor or several. This is forward-looking:
  talk and act the same regardless of head count. Do not attribute decisions to a named individual.
- **Deep Qt C++/QML expertise; assume familiarity with the codebase.** Don't over-explain things that
  are already well understood; focus on the problem at hand.
- **Prefers concise, direct responses.**

## Working style & collaboration

- **All UI/UX decisions are made by project leadership.** Implement what's asked; never propose or make
  unsolicited changes to QML appearance. (See `principles.md`, `feedback_pokered` never-change list below.)
- **Preserve existing notes/knowledge verbatim; don't silently "correct" them.** When integrating
  scattered files/comments into the notes, keep the original wording and flag any discrepancy as an open
  "verify before relying on this" question — annotate, don't rewrite. (Example: the Gen 1 growth-rate
  formulas in `growth-notes.txt` were not to be "fixed" — they may have been pasted for a reason and may
...
- **Use the task list HEAVILY — early, often, comprehensively (restated 2026-07-11, now a hard default).**
  ...
  evaporate mid-flight. The task list is watched live; it's a working artifact, not paperwork. It does
...
- **Run EVERYTHING in the background (2026-07-12, mandatory).** Builds, tests, captures, emulator runs,
  installs — hidden windows, headless/offscreen, detached + polled through a log. **Never steal the
  screen or focus.** If it can be captured headless and shown as an image instead, do that.
  See CLAUDE.md → Default Workflow.
- **…but DO open the app in the FOREGROUND, on the right screen, the moment it's ready to look
  at (2026-07-12, standing — the other half of the rule above).** Background is the default *while
  working*. The instant the work reaches "ready for a live pass", **launch it in front,
  already on the page in question, with the save loaded** — don't just say it's ready and leave it to be
  opened and navigated to manually. **Take the opportunity when it presents itself**, without being asked.
  Use the DEBUG harness flags to land directly on the right screen:
  `PokeredSaveEditor.exe --sav <save> --screen <name> [--select ...]`
  (`reference/dev-harness.md`). Background while building; foreground the second it's worth the time.
- **Manual screenshot review is mandatory on any UI change** — capture and actually scrutinise it
  before declaring done. See CLAUDE.md → Default Workflow step 1, `screenshots.md`, `ui-patterns.md`.
- **EVERY byte of the save must be editable — and NONE of them as a raw number (2026-07-12, standing).**
  ...
  1. **Nothing is hidden.** No field gets dropped from the UI because it is obscure, transient, or
     "not really map data". If the save holds it, the user can edit it. (Said in response to a proposal
     to simply not give the two Strength-push scratch bytes an editor. Wrong instinct — they get one.)
...
- **NO HIDDEN FIELDS (2026-07-15).** There shouldn't be hidden fields. Every field the save holds is
  **shown in place** — including the ones a real game rewrites on load, and the ones v1 never drew (the
  Character panel's trade-center bit was hidden in v1; it is now a first-class row). The ⚠ "the game
  rewrites this on load" mark is a **label on a visible row**, never a reason to tuck a field away. (This
  sharpens the warp/player panels' `[ ⚠ Show N fields the game rewrites ]` disclosure: a *collapsible
  grouping opened by one click* is fine, but a field that a view can never surface is not. When in doubt,
  show it.) Pairs with "every byte editable" above.
- **A briefed save-panel is built research → model-fix → probe → UI, across as many sessions as it
  takes — the UI is LAST and it is GATED (2026-07-15, standing).** When a new panel is briefed over a
  slab of save bytes (the Character panel — the nine map-global `AreaNPC` flags — was the occasion), do
  **not** rush to QML. The order is fixed and each step is its own phase, potentially its own session:
  **(1) research** to the primary source (`pret/pokered`) and write the `notes/reference/` note — real
  names, bytes, bits, who writes/reads, hack-value behaviour, and every place v1/v2's model is wrong;
  **(2) model-fix** — rename + persistence doc the ported class before any screen leans on it (the
  warp/sprite/player precedent); **(3) emulator probe** (`scripts/emu/`) for any load-bearing
  persistence claim a source-read alone can't settle — *the console is the oracle, a careful asm read
  has been wrong before*; **(4) the UI**, and only after the probe is green. "Break it into as many
  phases/sessions as needed" is the default, not a fallback: a 90%-done phase is not done. This is
  already CLAUDE.md doctrine (RESEARCH LANDS IN THE NOTES · DON'T BUILD WHAT HASN'T BEEN BRIEFED · do
  the LONG WORK); it is pinned here to be enforced. The worked example lives in
  `reference/npc-character-state.md` + `plans/map-screen.md` → Phase 9.
...
- **Don't auto-edit data JSON — ask first.** Any change to DATA in a `.json` (e.g.
  `projects/db/assets/data/*.json` — items, gameCorner, prices, maps, credits) needs project-leadership
  sign-off first; the JSON is curated game data (often hand-tuned against `pret/pokered`). Read freely;
  prefer fixing the C++/QML logic that *uses* the data. (E.g. the Game Corner rate `price: 20` was
  correct; the bug was the exchange math using it inverted — fixed the code, left the JSON alone.)
- **Save-file fidelity is sacred** — only ever write the exact bits/bytes for the requested edit; never
  normalize/repack/touch checksums or regions not explicitly named. (See `principles.md`.)
...
- **The ROM (`assets/references/backup.gb`) is a personal legal cartridge backup and must NEVER be
...
- Credits are easy to overlook — **proactively flag & add them** for any new asset, tool, service, data
  source, or AI assistance; don't wait to be asked. Credits live in
...
Project leadership owns the SemVer feel. Bump by the **scale/significance of the change to the product**,
not code/diff size (diff size is only a hint):
...
- **MAJOR (→ 1.0.0)** = project leadership only; never auto-bump.
...
- **Releases are MANUAL (2026-07-10).** Only cut a release (`--no-ff` merge `dev → main`) when project
  leadership says **"ship" / "ship it"**. Never auto-release even when green. `main` advances only by
  `--no-ff` tagged-release merges at a `VERSION` bump (PATCH direct; MINOR/MAJOR via `release/X.Y.0`).
  **CI owns the tag — never `git tag` by hand** (`release.yml` derives `v<VERSION>`; a manual tag makes
  the run skip). "Green" includes the remote `tests` CI, not just local `ctest`. See CLAUDE.md → Default
...
- **GitHub is part of normal management:** the `gh` CLI is installed + authed. When prepping to ship,
  check CI/issues/PRs and surface anything open; never auto-act on issues/PRs.
...
## Tooling & build/test (builds/tests/git run here)

- **Not sandbox-limited. Do not open a session claiming "no Qt tools available."** The
  `mcp__Windows-MCP__PowerShell` tool has real terminal access to the development machine with the full
  Qt 6.11 llvm-mingw toolchain — configure, build, run tests, launch the app, and git
  commit/push/merge directly, by default.
- **NEVER use the Cowork bash sandbox (`mcp__workspace__bash`) for writes** — banned outright (stale
  mounts / false truncation; a `sed -i` sweep once truncated 55 files on the real disk). Use
  **PowerShell** + Read/Write/Edit for everything real (git, builds, tests, lock-file cleanup). Bash
  reads can lag file-tool writes and show phantom truncation / false NUL bytes — verify with the **Read**
  tool, not `cat`/`grep`/`wc`.
...
- **`pret/pokered` disassembly** — the standing oracle for Gen 1 save-format / behaviour questions.
  ...
  outside the mount (needs PowerShell). Scan by label ranges, not just literal addresses (clears/copies
  hit regions by `label+size`). See `reference/gen1-knowledge.md`, `reference/box-recovery-research.md`.
- **pokered-save-editor v1** (Angular/Electron, TypeScript) at `assets/references/pokered-save-editor/` —
  ...
  Consult it; do NOT batch-port it wholesale. `assets/references/` is the official local-only reference
