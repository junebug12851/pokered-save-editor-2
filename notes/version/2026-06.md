...
### 2026-06-15 -- README screenshots via GitHub Pages; release-notes + release-policy standards

`.github/workflows/screenshots.yml` now deploys the UI screenshots to **GitHub Pages** (not a release,
not the git repo) — so the README embeds always-current images with **zero repo-size growth** and no
third party. On every push to `main` it captures headless (`screenshooter` under xvfb), gathers the
loose images (`screens/*.png` + `editor/*.png` + the GIFs, **excluding `frames/`**) into a small site +
gallery, and deploys to Pages. URLs: `https://junebug12851.github.io/pokered-save-editor-2/screenshots/<name>`.
(Rejected: a rolling images-only release — violates "releases are software-only"; an orphan branch —
bloats the repo object store; Imgur — purges images / deprecated API / needs a secret.) Also: every
