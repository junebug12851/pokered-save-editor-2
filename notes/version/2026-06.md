...
### 2026-06-15 -- Release pipeline: first-run shakeout fixes + GitHub in default management

First real `release.yml` run (`v0.7.2-alpha`): version / docs / Linux AppImage+tar.gz passed first try;
two shakeout fixes — `windeployqt --compiler-runtime` isn't supported on llvm-mingw (dropped it; the
libc++/libunwind/libwinpthread runtime DLLs are copied by hand), and `capture_screenshots.sh` now finds
the `screenshooter` binary under `build/tests/` on Linux (it's at the build root only on Windows). Also
wired GitHub into the default workflow (`CLAUDE.md`): the remote `tests` CI must be green before FF
