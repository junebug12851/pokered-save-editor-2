...
### 2026-06-15 -- Fix README top blockquote rendering in Doxygen (Pages docs home)

The README's intro blockquote rendered wrong as the Doxygen home page: a **two-line** blockquote using
`_..._` emphasis made Doxygen (a) not render the italics (literal underscores) and (b) **leak a literal
`</blockquote>`** that the following `---` turned into a stray `<h1>` (also polluting the nav/auto-TOC).
Fix: make it a **single-line** blockquote with `*...*` emphasis — Doxygen then renders `<em>` cleanly
with no leak, and the `---` stays. Verified with a local `doxygen Doxyfile` run (0 leaked tags, italics
present). GitHub renders it identically, so the README looks unchanged there. (A Doxygen-markdown gotcha,
noted in `reference/documentation.md`.)
