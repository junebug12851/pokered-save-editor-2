...
### 2026-06-15 -- Rolling `screenshots` release for README images (auto-updated by CI)

Added `.github/workflows/screenshots.yml`: on every push to `main` it captures the UI headless
(`screenshooter` under xvfb) and uploads the **loose** images — `screens/*.png` + `editor/*.png` + the
GIFs, **excluding `frames/`** — to a single rolling prerelease tagged `screenshots`, clobbering the old
ones. That gives each image a permanent, always-latest URL
(`…/releases/download/screenshots/<name>`) the README embeds, kept current by CI. Not a software
release (the versioned `release.yml` artifacts are the downloads). `VERSION` unchanged (CI-only).
