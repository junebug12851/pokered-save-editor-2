...
not the git repo): `/docs/` (the Doxygen documentation) and `/screenshots/<name>` (the UI images + GIFs
+ a gallery), with a small landing page. So the README embeds always-current images and the docs are
browsable online, with **zero repo-size growth** and no third party. It captures headless
(`screenshooter` under xvfb; `screens/*.png` + `editor/*.png` + GIFs, **excluding `frames/`**), runs
`doxygen`, and deploys via `configure-pages`/`upload-pages-artifact`/`deploy-pages`. Docs reference
screenshots by the absolute Pages URL (works on Pages, GitHub, and the README alike). README hero +
Screenshots section now use these Pages URLs instead of the old Imgur links. (Rejected for images: a
