...
### 2026-06-15 -- Screenshot tool: capture at the user's real window size (~751×480)

The screenshots kept looking "too big" because they were captured at 1130×740 — but that's only the
fresh-profile default; the app is designed for a small rectangle and is actually run at ~751×480. The
tool now reads the real saved size from `QSettings("Twilight","Pokered Save Editor") → WindowState/size`
and resizes to it (override `PSE_SHOT_SIZE="WxH"`, fallback 1130×740), resizing **before** `start()` so
the QML lays out once at the final size (a late resize had left Home's greyed Maps/Options tiles
unrendered). Shots now match the app's actual window. Tooling only. See `notes/reference/screenshots.md`.
